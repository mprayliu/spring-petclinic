...
"""
  ...
  stages {
    ...
    stage('Image Vulnerability Scan') {
      steps {
        writeFile file: 'anchore_images', text: "${env.OCIR_URL}/${env.OCIR_NS}/samples/spring-petclinic:v1.0.${env.BUILD_ID}"
      ...
      steps {
        container('maven') {
          sh """
            ...
            cd deploy
            # update values.yaml
            sed -i -r 's,repository: (.+),repository: ${env.OCIR_URL}/${env.OCIR_NS}/samples/spring-petclinic,' values.yaml
